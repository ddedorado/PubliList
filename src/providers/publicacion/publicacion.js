var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { Injectable } from '@angular/core';
import { Http } from '@angular/http';
import 'rxjs/add/operator/map';
//AngularFirebase2 Module
import { AngularFire } from 'angularfire2';
var PublicacionService = (function () {
    function PublicacionService(http, af) {
        this.af = af;
        this.publicacionesObservable = af.database.list('/publicaciones');
    }
    PublicacionService.prototype.getPublicaciones = function () {
        return this.publicacionesObservable;
    };
    PublicacionService.prototype.addPublicacion = function (data) {
        this.publicacionesObservable.push({
            title: data.title,
            code: data.code
        });
    };
    PublicacionService.prototype.deletePublicacion = function (id) {
        this.publicacionesObservable.remove(id);
    };
    return PublicacionService;
}());
PublicacionService = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [Http, AngularFire])
], PublicacionService);
export { PublicacionService };
//# sourceMappingURL=publicacion.js.map